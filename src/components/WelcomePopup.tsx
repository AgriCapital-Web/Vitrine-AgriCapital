...
const WelcomePopup = () => {
  ...
  const openSubscriberPortal = () => {
    window.open('https://app.agricapital.ci/abonne', '_blank');
  };
  ...
  return (
    ...
    >
      ...
      >
        {/* Countdown close button - VISIBLE on top right */}
        <button
          onClick={handleClose}
          disabled={!canClose}
          className={`absolute top-2 sm:top-3 ${isRTL ? 'left-2 sm:left-3' : 'right-2 sm:right-3'} z-30 w-12 h-12 sm:w-14 sm:h-14 rounded-full transition-all shadow-lg ${
            canClose 
              ? 'bg-white/30 hover:bg-white/40 cursor-pointer hover:scale-110' 
        ...
        >
          ...
          <span className={`absolute inset-0 flex items-center justify-center text-base sm:text-lg font-bold ${canClose ? 'text-green-400' : 'text-white'}`}>
            {canClose ? <X className="w-5 h-5 sm:w-6 sm:h-6" /> : countdown}
          </span>
        </button>
